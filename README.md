...
- On Windows, every daemon data directory, descriptor, ownership secret, session directory, metadata file, and output file has inheritance removed and grants Full Control only to the current user SID and LocalSystem. This applies to a custom `PTY_DAEMON_DIR` too. If the current user SID cannot be resolved or `icacls.exe` cannot apply that DACL, daemon startup fails before publishing its descriptor.
