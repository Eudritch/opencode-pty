...
- `session.deleted` stops only that owner's `conversation` sessions, including native exec records. `persistent` sessions remain until their owner explicitly stops them. Controlled daemon restarts reconnect reachable native workers; workers independently enforce their direct-child timeout and output cap.
