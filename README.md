...
The experimental Bash command is opaque. It evaluates only the complete original string under `bash`, then uses native `ctx.ask`; it does not parse shell arguments, create approval requests, or honor companion grants. It cannot claim parser equivalence with OpenCode Bash. An explicit `workdir` is canonicalized against the worktree boundary where available; outside paths use the host-style parent wildcard `/path/*` for `external_directory` approval.
...
Before every `pty_spawn` and `shell_exec`, this plugin reads the merged configuration and locally evaluates ordered global then agent rules. An effective local `deny` is final. An `allow` proceeds; `ask` and unmatched rules invoke native `ctx.ask` using the exact executable-plus-argv `bash` pattern. External workdirs are checked against the Git worktree boundary where available and use the host-style parent wildcard `/path/*`; local deny remains final, while other decisions invoke native `ctx.ask`. The SDK cannot expose the host shell AST scanner, so advanced argv tools cannot match native Bash parsing semantics.
