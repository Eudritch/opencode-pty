...
| `bash` | Experimental opaque foreground compatibility override, opt-in only. |
...
`shell_exec` is `exec` mode, not a shell parser: `command` and `args` are passed as argv and a positive timeout is required. A Rust per-session worker owns every child, exposes an authenticated loopback RPC endpoint, and writes redacted output to the existing session chunk journal. The daemon resolves its platform-specific worker from the installed package, so no repository path is required. After restart, only an explicitly `persistent` session is eligible to reconnect to a reachable worker and recover its output cursor and final state.

Native OpenCode Bash remains unmodified by default and is the only path with OpenCode's exact shell parsing and effective session-policy behavior. Explicitly set the plugin option `{ "bash": true }` only to enable an experimental opaque compatibility override. It duplicates OpenCode's Bash ID, requires a host that permits duplicate IDs, and runs a finite foreground shell: Windows uses `%ComSpec% /d /s /c`, while POSIX uses `/bin/sh -lc`. `timeout` is milliseconds, defaults to 120000, and rounds down to the daemon's whole-second timeout (sub-second values are rejected).

The experimental Bash command is opaque. It evaluates only the complete original string under `bash` and does not parse shell arguments, so it cannot claim parser equivalence with OpenCode Bash. A local policy `ask` publishes a daemon-mediated approval request that the TUI companion may claim within a short lease and resolve; unclaimed or expired requests fall back to the host's native `ctx.ask`. Session-scoped approvals become grants bound to the requesting agent and are reused for matching commands. An explicit `workdir` is canonicalized against the worktree boundary where available; outside paths use the host-style parent wildcard `/path/*` for `external_directory` approval.

`pty_spawn` is `pty` mode and remains interactive. Unix workers use a real controlling terminal with merged redacted UTF-8 terminal output; `pty_resize` changes its columns and rows. There is no screen emulator, raw byte API, or screen snapshot. A supplied `idempotencyKey` reuses only a matching active PTY for the full originating owner identity and canonical workdir; changing command, args, environment, timeout, or name is rejected. Titles and descriptions are presentation fields and do not affect reuse. `pty_wait` conditions are literal output, a limited-safe regex, or exit. They run in the daemon against output/exit events with a 3600-second maximum deadline, not plugin polling. `pty_send_wait` captures its output boundary immediately after PTY input is accepted, so earlier output cannot satisfy the wait while an immediate reply can. Output readiness is evidence only; no bare `ready` state is claimed.
...
- `session.deleted` stops and removes only that owner's `conversation` sessions, including native exec records. `persistent` sessions remain until their owner explicitly stops and cleans them up. Before a controlled daemon restart publishes its descriptor, active `conversation` sessions are marked for cleanup; they reconnect only to stop and reap, while failed reaping remains a readable owner-scoped lost tombstone. Only `persistent` workers whose descriptor, endpoint, token fingerprint, and authenticated health identity match are eligible to resume; incompatible records remain lost and readable. Exec workers enforce their direct-child timeout and aggregate output limit; PTYs have no worker deadline unless one is supplied and discard oldest journal output at the configured aggregate retention cap. A worker terminal result is complete only after its direct child exits and both stdout/stderr readers reach EOF; a bounded reader-drain timeout reports incomplete output as unknown rather than complete. `directChildExited` is direct-child evidence, not descendant containment evidence.
- Environment defaults to a small platform/project-safe allowlist plus explicitly supplied variables. On Windows, the safe profile includes `APPDATA` and `LOCALAPPDATA`, so normal stored `gh` and Git Credential Manager credentials work in safe exec. `pty_spawn` and `shell_exec` reject custom `env` or `inheritEnv: true` under `ask` or unmatched policy; configure an explicit local `bash` `allow` rule before using either. First-time interactive login therefore requires that explicit allow as well as `inheritEnv: true`; no noninteractive command can prompt. Raw environment values are never persisted: records retain only profile kind, redacted key markers, and a fingerprint. Output replaces values of obvious secret-named environment variables with `[REDACTED]` before PTY journal or exec output persistence. The streaming redactor retains at most 4095 UTF-8 characters (secret values are capped at 4096 bytes), so ordinary trailing output can wait for a later callback or process exit.
- Browser-facing APIs, WebSockets, and slash commands are intentionally not provided.

Before every `pty_spawn` and `shell_exec`, this plugin reads the merged configuration and locally evaluates ordered global then agent rules. An effective local `deny` is final. An explicit local `allow` proceeds; `ask` and unmatched rules invoke native `ctx.ask` using the exact executable-plus-argv `bash` pattern, except they reject custom or inherited environments before asking because approval metadata cannot bind those execution options. For the experimental Bash override only, an `ask` decision is first handed off to the TUI companion as a short-lived daemon approval request; when it is not claimed within the lease, the plugin falls back to the host's native `ctx.ask`. External workdirs are checked against the Git worktree boundary where available and use the host-style parent wildcard `/path/*`; local deny remains final, while other decisions invoke native `ctx.ask`. The SDK cannot expose the host shell AST scanner, so advanced argv tools cannot match native Bash parsing semantics.
...
The default `opencode-pty` entry remains the server plugin. The optional TUI companion adds a compact `sidebar_content` panel for PTYs owned by the active OpenCode session. Its approval controls apply to explicitly UI-controlled advanced PTY approval operations and to claiming experimental Bash `ask` requests within their short lease; they never authorize native Bash and cannot bypass a local deny. A Bash request the companion does not claim in time falls back to the host's native `ctx.ask`. It fetches the routed session through the supported OpenCode client API, then requires its canonical directory to match `api.state.path.directory`; it shows nothing and performs no approval action when that check fails.
...
The root entry preserves native Bash behavior. Set `{ "bash": true }` on the server entry only to enable the experimental Bash compatibility override; `{ "bash": false }` remains accepted and also leaves native Bash untouched.
