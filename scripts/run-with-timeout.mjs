...
export async function runWithTimeout({ command, args = [], timeoutMs, signal } = {}) {
  if (typeof command !== "string" || !command || !Array.isArray(args) || !args.every((arg) => typeof arg === "string")) {
  ...
  if (signal !== undefined && (signal === null || typeof signal !== "object" || typeof signal.addEventListener !== "function" || typeof signal.removeEventListener !== "function" || typeof signal.aborted !== "boolean")) {
  ...
  return {
    ...
    status: statusFor({
      ...
      directChildObserved,
      childSpawned,
      cleanupUnconfirmed: cleanup.directChildCleanupAttempted && !cleanup.directChildCleanupConfirmed,
    }),
    timedOut,
    cancelled: wasCancelled,
    elapsedMs: Math.round(performance.now() - started),
    stdout: stdout(),
    stderr: finalResult.error ? `${capturedStderr}${capturedStderr ? "\n" : ""}${finalResult.error}` : capturedStderr,
...
function assertResultSchema(result, label) {
  ...
  assert.equal(typeof result.exitCode === "number" || result.exitCode === null, true, `${label} exitCode is invalid`);
  assert.equal(typeof result.signal === "string" || result.signal === null, true, `${label} signal is invalid`);
  ...
  assert.equal(typeof result.outputMayBePartial, "boolean", `${label} outputMayBePartial is invalid`);
  assert.equal(typeof result.directChildCleanupAttempted, "boolean", `${label} directChildCleanupAttempted is invalid`);
  assert.equal(typeof result.directChildCleanupConfirmed, "boolean", `${label} directChildCleanupConfirmed is invalid`);
}

async function selfTest(signal) {
  const finite = await runWithTimeout({ command: process.execPath, args: ["-e", "console.log('ok')"], timeoutMs: 2_500, signal });
  assertResultSchema(finite, "finite");
  if (process.platform !== "win32") {
    assert.equal(finite.status, "not_started", "unsupported platform started a child");
    assert.match(finite.stderr, /supported only on Windows/, "unsupported platform was not reported");
    process.stdout.write(`${JSON.stringify({ ok: true, unsupported: finite })}\n`);
    return;
  }

  const timedOut = await runWithTimeout({ command: process.execPath, args: ["-e", "setInterval(() => {}, 1_000)"], timeoutMs: 500, signal });
  const cancellation = new AbortController();
  const cancelledPromise = runWithTimeout({ command: process.execPath, args: ["-e", "setInterval(() => {}, 1_000)"], timeoutMs: 2_500, signal: cancellation.signal });
  ...
  assert.equal(timedOut.directChildCleanupConfirmed, true, "timed out direct child cleanup was not confirmed");
  ...
  assert.equal(cancelled.directChildCleanupAttempted, true, "aborted direct child cleanup was not attempted");
  assert.equal(cancelled.directChildCleanupConfirmed, true, "aborted direct child cleanup was not confirmed");
  const preAbort = new AbortController();
  preAbort.abort();
  const preAborted = await runWithTimeout({ command: process.execPath, timeoutMs: 2_500, signal: preAbort.signal });
...
try {
  if (process.argv[2] === "--self-test") {
    ...
    process.stdout.write(`${JSON.stringify(await runWithTimeout({ ...input, signal: controller.signal }))}\n`);
  }
} catch (error) {
