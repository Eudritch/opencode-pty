...
The project has focused daemon, storage, plugin-client, approvals, TUI-state, worker-target, source-daemon crash, and package smoke tests. `test/daemon.test.ts` is a large integration-heavy suite. Rust tests live in `worker/src/main.rs`. This is meaningful coverage, particularly for storage corruption, ownership RPC checks, recovery, and Linux containment, but it is not a green cross-platform release gate today.
...
Phase 3 bootstrap hardening has focused local coverage: values over 4,096 UTF-8 bytes fail before session admission, an exact 4,096-byte Unicode sensitive value remains redacted at EOF, worker startup stderr is piped, bounded, and redacted, and a custom worker path fails without `PTY_NATIVE_WORKER_DEV=1`. Rust unit tests repeat the byte-boundary and multibyte-EOF checks. Native input pressure, Windows journal contention, and batch-policy binding remain open.

After this slice, direct Bun 1.3.14 `bun test` and script-entry Bun 1.3.6 `bun unittest` each pass **218 tests / 16 skips / 796 expectations**; Rust format/test (15)/clippy, TypeScript, and fresh-package smoke pass. Expected corruption/recovery warnings and injected orphan-reaping diagnostics remain test evidence, not failures.

Phase 2 first-slice checks pass locally: `cargo fmt --check`, 13 Rust worker tests, `cargo clippy -D warnings`, `bun format`, `bun typecheck`, direct Bun 1.3.14 `bun test` (193 pass, 16 skipped, 723 expectations), script-entry Bun 1.3.6 `bun unittest` (193 pass, 16 skipped, 723 expectations), `bun package:smoke`, and `git diff --check`. Focused mocked-worker admission coverage proves concurrent same-owner cap enforcement, different-owner isolation, idempotent PTY reuse at capacity, immediate release on first-record failure, retention after uncertain start, strict terminal/no-child release, pre-activation cleanup, metadata-only list reads, and admission independence from an unrelated stalled worker snapshot.

Phase 2 controller and boundary-split checks pass locally: direct Bun 1.3.14 `bun test` and project Bun 1.3.6 `bun unittest` report 197 pass, 16 skipped, and 734 expectations; `bun package:smoke` also passes. Focused tests prove reservation behavior survives its registry extraction; journal paging/search/recovery semantics survive their reader extraction; write/stop/resize use one reproducible router lane; terminal finalization cannot race a later queued observer; a foreign owner cannot trigger a routed worker snapshot; and `stop`, synchronous disposal, and async disposal remove the same descriptor. The controller test uses settled results rather than a deliberately unresolved mock write, so a timeout cannot leave a live test task that prevents Bun from exiting.

The Windows checkpoint gate exposed a stale recovery-lock race: a quarantined lock can disappear before a defensive hard-link restore. `acquireStartLockRecovery()` now treats that `ENOENT` as ordinary contention and retries. Its deterministic regression, full direct Bun 1.3.14 suite, and project Bun 1.3.6 suite pass at 198 pass, 16 skipped, and 735 expectations; `bun package:smoke` passes.

V2 is the only `session.json` writer: lifecycle fields live beneath one discriminated `state`, not alongside root status/worker/cleanup flags. Explicit V0, V1, and V2 decoders are covered. V0/V1 terminal imports require a canonical owner tuple, lowercase SHA-256 capability hash, explicit direct-child exit, and explicit drained containment; their stream drain is recorded as unknown. Live or unproven legacy records become non-runnable cleanup tombstones retaining output. Malformed known V2 records quarantine, while explicit V0, future, incomplete, and null-owner artifacts stay byte-for-byte untouched. Pure reducer coverage proves terminal-to-unreachable payload retention and rejects cleaning revival. `SessionRegistry` now bounds active sessions, durable records, pending waits, queued controller-lane input, and aggregate retained-output reservations; diagnostics reports limit and owner/global usage. New direct Bun 1.3.14 and script-entry Bun 1.3.6 suites pass 214 tests with 16 skips and 786 expectations; the fresh-package smoke contract passes.

Prepared-worker tests prove child creation is gated behind durable reference persistence: a real marker command remains absent before `client.start()`, both supervisor start paths block at the reference write, and a failed reference write rolls back before activation while retaining the descriptor. A pre-start rollback writes a strict reference-bound no-child receipt; recovery rejects a forged receipt, deletes only after valid receipt verification, and a post-start worker never emits one. A retained descriptor plus a strict `spawn-failure.json` proves the separate post-start case where `start` was accepted but no direct child was created. A persisted no-worker no-child failure is reaped at restart; every ambiguous startup failure is V2 unreachable instead of a false terminal claim. Windows `job_assign` reports `directChildStarted:true` and is excluded from that no-child path. Terminal conversation cleanup records an in-memory fresh-shutdown proof so normal finalization deletes without a redundant reconnect, while crash-recovered terminal sessions still require a fresh authenticated shutdown. The source-daemon crash test adds the live-child boundary: it kills the daemon after a persistent child records one marker, restarts, confirms authenticated `running` recovery, and proves the marker remains one line. Process termination cannot prove power-loss ordering, and the published-platform crash/pressure matrix remains Phase 4 work.
