...
Phase 2 controller checks pass locally: direct Bun 1.3.14 `bun test` and project Bun 1.3.6 `bun unittest` report 196 pass, 16 skipped, and 732 expectations; `bun package:smoke` also passes. Focused tests prove write/stop/resize use one reproducible lane, terminal finalization cannot race a later queued observer, and `stop`, synchronous disposal, and async disposal remove the same descriptor. The controller test uses settled results rather than a deliberately unresolved mock write, so a timeout cannot leave a live test task that prevents Bun from exiting.
