...
| OQ-07 | Can current storage format be migrated in place, and are persisted sessions from released versions supported? | State redesign needs a deletion/migration boundary. | Locally resolved: V2 is the only writer with explicit V0/V1/V2 decoding; proven legacy terminals import with unknown drain while live/unproven records become cleanup-only tombstones. Deterministic pre-start/reference/post-start/finalization coverage and a source-daemon live-child restart test cover the local cutover. Explicit V0, future, incomplete, and null-owner artifacts remain unchanged. | Repeat the crash and migration drivers on every published target in Phase 4. |
