...
| OQ-07 | Can current storage format be migrated in place, and are persisted sessions from released versions supported? | State redesign needs a deletion/migration boundary. | Partially resolved: V2 is the only writer with explicit V0/V1/V2 decoding; proven legacy terminals import with unknown drain while live/unproven records become cleanup-only tombstones. Pre-activation references, fresh terminal-shutdown proof reuse, and strict pre-start/post-start no-child receipts are implemented. Explicit V0, future, incomplete, and null-owner artifacts remain unchanged. | Complete the crash matrix around preparation, reference persistence, start-frame delivery, child creation, and terminal persistence. |
