...
**Status:** In progress. The locally green control-plane slices delete legacy daemon-side direct exec, make `list()` metadata-only, replace global admission polling with supervisor-owned full-owner reservations, serialize session mutations through controller lanes, and unify daemon shutdown/disposal. The planned responsibility split remains.
...
The first two completion conditions, controller lanes, and unified shutdown/disposal are locally verified. The phase remains open for the planned responsibility split.
