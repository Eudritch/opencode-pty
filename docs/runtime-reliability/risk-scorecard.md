...
| Security | 3 | Strong loopback/capability/DACL controls; full-owner reuse, owner-safe V0 decoding, tombstone retention, bounded sensitive bootstrap values, development-only worker overrides, and fail-closed dynamic environments improve isolation, but batch-policy binding and native input behavior remain open. |
| Observability | 3 | Structured records and diagnostics exist, and worker stderr is now bounded/redacted rather than inherited, but lifecycle events are not a concise authoritative stream. |
