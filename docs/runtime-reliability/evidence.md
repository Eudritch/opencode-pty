...
| Phase 3 bootstrap hardening | Values above 4,096 UTF-8 bytes fail before admission and again in Rust; an exact 4,096-byte Unicode value is redacted at EOF; worker stderr is piped into a bounded redacted tail; and source-worker paths require explicit development mode. Direct Bun 1.3.14 and script-entry Bun 1.3.6 each passed 218 tests with 16 skips and 796 expectations; Rust format/test (15)/clippy, TypeScript, and package smoke passed. | Fact | One Windows x64 host; native input pressure, worker journal contention, batch-policy binding, and platform matrix remain open. |
