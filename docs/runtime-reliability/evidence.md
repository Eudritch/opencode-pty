...
| V0-to-V1 record migration | Raw V0 terminal records were rewritten as V1; raw V0 live and unproven-terminal records became non-runnable cleanup tombstones; unknown future versions and incomplete owner tuples failed in place without quarantine or rewrite | Fact | Local decoder/storage coverage, not a crash-point or historical-data corpus. |
...
| Current source/package regression | Direct Bun 1.3.14 `bun test` and script-entry Bun 1.3.6 `bun unittest` each passed 189 tests with 16 skips and 712 expectations; `bun package:smoke` passed | Fact | One Windows x64 host. |
...
The implemented scope is deliberately narrow: the existing persisted statuses remain compatible, while a pure reducer prevents stale transition/fact overwrite. A root V1 record marker is now required for new writes. The storage decoder classifies unversioned V0 before validation or recovery: only V0 records with a canonical owner tuple, a lowercase SHA-256 capability hash, explicit direct-child exit, and explicit drained containment remain terminal. All other supported V0 records become `lost` conversation tombstones with `pendingCleanup` and retain `output.log`; proven terminal imports use a persisted cleanup marker so source-log deletion is retryable. Null, incomplete, or future-version artifacts are skipped untouched, so they cannot block healthy sessions or become actionable. Persisted V1 terminal cleanup requires the same explicit direct-child and containment evidence. Daemon-start recovery marks conversations for cleanup before descriptor publication; only protocol-compatible persistent sessions can make the authenticated `lost -> recovered` transition. A prepared worker that exits before `start` can instead provide a strict receipt bound to its persisted reference; that sole no-child evidence permits deletion. A worker that accepted `start` but never created a child can prove that only with a retained descriptor plus a strict `spawn-failure.json`; Windows child-started failures are now correctly excluded from that path. Lost records otherwise remain readable owner-scoped tombstones until strict cleanup evidence exists.
