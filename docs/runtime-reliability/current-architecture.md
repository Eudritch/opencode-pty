...
| Supervisor | Records, worker references, recovery, finalization queues, reads/waits, lifecycle mapping, and per-owner admission reservations | `src/daemon/supervisor.ts` | Necessary responsibilities, but still concentrated; controller lanes and narrower collaborators remain Phase 2 work. |
